...
author: "Ally Deremer, Eshna Prakash, Jose Lopez, Jasper Cheng"
date: "3/5/2021"
...
ggplot(usa, aes(x=time,y=New_cases))+
  geom_point()+
  labs(title="Plot of New Cases over time for USA",x="Time (in days)", y = "New Cases")

ggplot(usa, aes(x=time,y=New_deaths))+
  geom_point()+
  labs(title="Plot of New Deaths over time for USA",x="Time (in days)", y = "New Deaths")

ggplot(usa, aes(x=time,y=Cumulative_deaths))+
  geom_point()+
  labs(title="Plot of Cumulative Deaths over time for USA",x="Time (in days)", y = "Cumulative Deaths")

ggplot(usa, aes(x=time,y=Cumulative_cases))+
  geom_point()+
  labs(title="Plot of Cumulative Cases over time for USA",x="Time (in days)", y = "Cumulative Cases")
...
nigeriap=data.frame(yfit=predict(nigeria2,nigeria),xfit=nigeria$time)
ggplot(nigeria,aes(x=time,y=CFR))+
  geom_point()+
  geom_line(nigeriap,mapping=aes(x=xfit,y=yfit),col="blue")+
  labs(title="CFR vs Time",x="Time")
...
```{r}
#For US
us1= lm(usa$CFR~poly(usa$time, degree=1, raw=T))
us3= lm(usa$CFR~poly(usa$time, degree=3, raw=T))
us4= lm(usa$CFR~poly(usa$time, degree=4, raw=T))
us50= lm(usa$CFR~poly(usa$time, degree=50, raw=T))

#plot(usa$time, usa$CFR)
#lines(smooth.spline(usa$time,predict(us2)),col='red')

usa1=data.frame(yfit=predict(us1,usa),xfit=usa$time)
usa3=data.frame(yfit=predict(us3,usa),xfit=usa$time)
usa4=data.frame(yfit=predict(us4,usa),xfit=usa$time)
usa50=data.frame(yfit=predict(us50,usa),xfit=usa$time)

ggplot(usa,aes(x=time,y=CFR))+
  geom_point()+
  labs(title="USA, CFR vs Time",x="Time")+
  geom_line(usa1,mapping=aes(x=xfit,y=yfit),col="red")+
  geom_line(usa3,mapping=aes(x=xfit,y=yfit),col="blue")+
  geom_line(usa4,mapping=aes(x=xfit,y=yfit),col="green")+
  geom_line(usa50,mapping=aes(x=xfit,y=yfit),col="purple")
...
russiap=data.frame(yfit=predict(russpoly.model,nigeria),xfit=russia$time)
ggplot(russia,aes(x=time,y=CFR))+
  geom_point()+
  geom_line(russiap,mapping=aes(x=xfit,y=yfit),col="red")+
  labs(title="Russia, CFR vs Time",x="Time")
```
##Turkey Model
```{r}
#For Turkey
...
turkeyp=data.frame(yfit=predict(turkpoly.model,nigeria),xfit=russia$time)
ggplot(turkey,aes(x=time,y=CFR))+
  geom_point()+
  geom_line(turkeyp,mapping=aes(x=xfit,y=yfit),col="red")+
  labs(title="Turkey, CFR vs Time",x="Time")
```
...
japanp=data.frame(yfit=predict(modjapan2,japan),xfit=japan$time)
ggplot(japan,aes(x=time,y=CFR))+
  geom_point()+
  geom_line(japanp,mapping=aes(x=xfit,y=yfit),col="red")+
  labs(title="Japan, CFR vs Time",x="Time")
```
##China Model
```{r}
...
# Conclusion
This report aimed to investigate how Case Fatality Rate (CFR) for COVID-19 changes across time for 12 of the most populated countries in the world (2 most populated countries per WHO region), specifically over the first hundred days after the number of cumulative cases hits 100. Through a series of statistical analysis, this report was able to analyze the hypothesis stated earlier.

It was hypothesized that the trends of CFR will vary significantly across these countries. The results of this report matches with the initial hypothesis. Through fitting the 12 observed countries’ CFR across time, a variety of regressions were chosen that fits the data the best. These polynomial regressions vary in correlation and degree of polynomials, which visually supports the hypothesis that CFR across time will be different for the countries. 

For statistical and quantitative proof, employing ANOVA compares the polynomial regression that is not blocked by countries to the contrary. Since these regressions have a degree mainly of 3 and 4, these polynomial regressions were chosen for the ANOVA analysis. With an extremely low p-value, it was proven that countries do have a significant statistical effect on CFR across time, for both polynomial degrees of 3 and 4. Therefore, demonstrating CFR over time to not be constant over different countries at a highly significant level. 

In order to be more specific and explore beyond the big picture, comparing the country’s regression lines side by side informs the difference between countries’ CFR over time. The use of simultaneous confidence bands, which shows the 95% confidence interval across the whole regression line, appears that even though some countries do share a similar trend and overlaps, the majority of regressions were very different from one another.  Hence, once again proving CFR trends over time is different for each country.

Looking at these regressions may reflect how countries are able or not able to react in controlling the virus. It is observed that most CFR trends are fluctuating across the timeframe. A drop after a peak might be due to the implementation of a certain policy or a country gaining better knowledge on how to contain and treat the virus. While an increase after a trough may be hints of hitting medical capacity or loosening of policies. There are many external factors and reasons to explore beyond numbers of COVID cases and deaths. These are all topics that can be delved deeper in further and more complicated analysis, however are all mere guesses at the current analysis.
...
Used codes on confidence interval bands : https://stats.stackexchange.com/questions/231059/compare-the-statistical-significance-of-the-difference-between-two-polynomial-re
